...
merge m:1 idhog using ensanut_f1_informacion_general.dta, ///
...
gen noasis_=0 if (pd16==1 & pd03>=6 & pd03<=12)
replace noasis_=1 if (pd16==2 & pd03>=6 & pd03<=12)
egen    noasis = sum (noasis_) if viv_parti==1, by(idhog)

cap drop hog_noasis
gen hog_noasis= 0 if  noasis==0
...
use ensanut_f1_personas, clear
merge m:1 idhog using ensanut_f1_vivienda, keepusing(quint)
drop _merge
