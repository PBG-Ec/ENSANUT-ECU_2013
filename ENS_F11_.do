...
lab def edad_cat11 1 "1-3 años" 2 "4-8 años" 3 "9-13 años"  ///
		  4 "14-18 años" 5 "19-30 años" 6 "31-50 años" 7 "mas de 50 años",replace
...
************************************
*Adecuación de vitamina b12 SIN AJUSTAR
...
************************************
*Adecuación de zinc sin ajustar
...
**************************************
*Adecuación de Hierro (Metodo probabilístico) National Research Council
...
*Por edad
*Categorias de edad con sexo para niños menores de 4 años (hierro)
...
*CONTRIBUCIÓN PORCENTUAL DE MACRONUTRIENTES POR GRUPO ÉTNICO
local V por_prot por_cho por_grasa por_grasasat
foreach Y in `V' {
	tabstat `Y'  [aw= pw], by(gr_etn) ///
	  stat(n mean p50 p25 p75 p90 p95 sem sd) c(statistics)
...
*CONTRIBUCIÓN PORCENTUAL DE MACRONUTRIENTES POR SUBREGION
local V por_prot por_cho por_grasa por_grasasat
foreach Y in `V' {
	tabstat `Y'  [aw= pw], by(subreg) ///
	  stat(n mean p50 p25 p75 p90 p95 sem sd) c(statistics)
...
*CONTRIBUCIÒN PORCENTUAL DE MACRONUTRIENTES POR QUINTIL
local V por_prot por_cho por_grasa por_grasasat
foreach Y in `V' {
	tabstat `Y'  [aw= pw], by(quint) ///
	  stat(n mean p50 p25 p75 p90 p95 sem sd) c(statistics)
...
*CONTRIBUCIÒN PORCENTUAL DE MACRONUTRIENTES POR GRUPO DE EDAD SEXO
local V por_prot por_cho por_grasa por_grasasat
foreach Y in `V' {
	tabstat `Y'  [aw= pw], by(edad_cat11) ///
	  stat(n mean p50 p25 p75 p90 p95 sem sd) c(statistics)
...
*CONTRIBUCIÒN PORCENTUAL DE MACRONUTRIENTES POR zonas de planificacion
local V por_prot por_cho por_grasa por_grasasat
foreach Y in `V' {
	tabstat `Y'  [aw= pw], by(zonas_planificacion) ///
	  stat(n mean p50 p25 p75 p90 p95 sem sd) c(statistics)
...
lab def edadc 1 "1 a 3 años hombres" 2 "1 a 3 años mujeres" ///
  3 "4 a 8 años hombres " 4 "4 a 8 años mujeres " 5 "9 de 13 años hombres" ///
  6 "9 de 13 años mujeres" 7 "14 a 18 años hombres" 8 "14 a 18 años mujeres" ///
  9  "19 a 30 años hombres" 10 "19 a 30 años mujeres" ///
  11 "31 a 50 años hombres" 12 "31 a 50 años mujers" ///
  13 "51 a 60 años hombres" 14 "51 a 60 años mujeres"
lab val edadc edadc
